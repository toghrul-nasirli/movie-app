    ...
    public function movie()
    {
        return collect($this->movie)->merge([
            ...
            'cast' => collect($this->movie['credits']['cast'])->map(
                fn ($cast) => collect($cast)->only(['profile_path', 'name', 'character'])
            )->take(5),
            'images' => collect($this->movie['images']['backdrops'])->map(
                fn ($cast) => collect($cast)->only('file_path')
            )->take(9),
            'trailer' => 'https://www.youtube.com/embed/' . $this->movie['videos']['results'][0]['key'],
