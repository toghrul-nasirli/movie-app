<div x-data="{ isOpen: true }" @click.away="isOpen = false" @keydown.escape.window="isOpen = false" @keydown.shift.tab="isOpen = false" @keydown="isOpen = true" class="relative mt-3 md:mt-0">
    <input wire:model.debounce.500ms="search" x-ref="search" @keydown.window.prevent="if(event.keyCode === 191) { $refs.search.focus(); }" @focus="isOpen = true" type="text" class="bg-gray-800 text-sm rounded-full w-64 px-4 pl-8 py-1 focus:outline-none focus:shadow-outline" placeholder="Search (Press '/')">
